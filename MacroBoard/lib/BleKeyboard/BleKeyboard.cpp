...
BleKeyboard::BleKeyboard(std::string deviceName, std::string deviceManufacturer, uint8_t batteryLevel) 
    ...
    , current_asciimap(asciimap_en_gb) {}
...
void BleKeyboard::setLayout(int layout_id)
{
  if (layout_id == 1) {
    current_asciimap = asciimap_en_us;
  } else if (layout_id == 2) {
    current_asciimap = asciimap_pt_br;
  } else {
    current_asciimap = asciimap_en_gb;
  }
}
...
size_t BleKeyboard::press(uint8_t k)
{
	...
	if (k >= 136) {			// it's a non-printing key (not a modifier)
		...
		k = pgm_read_byte(current_asciimap + k);
		if (!k) {
			setWriteError();
			return 0;
		}
